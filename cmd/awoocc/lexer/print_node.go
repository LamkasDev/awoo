...
func PrintNodeTypeArray(settings *AwooLexerSettings, context *lexer_context.AwooLexerContext, n *node.AwooParserNode) string {
	arrayType := node.GetNodeTypeArrayType(n)
	arraySize := node.GetNodeTypeArraySize(n)
